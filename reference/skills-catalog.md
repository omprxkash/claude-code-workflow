...
[skills CLAUDE.md template](../templates/system-prompts/skills-system.md) for the full architecture.
...
| Skill | What it does |
|---|---|
| `design-website` | Generate a premium mockup website for a prospect (bold editorial-agency style); inputs from Google Sheet. |
...
## AIOS & knowledge management

| Skill | What it does |
|---|---|
| `grill-me` | Structured interview skill — extracts knowledge about you, your business, or a project through relentless one-at-a-time questions. Saves output to `brainstorm/` as context files. Use to bootstrap an AIOS or start any project with rich context. |
| `session-handoff` | Pre-`/clear` snapshot skill — generates a full handoff doc (what was built, decisions made, file paths, what comes next) so the next session picks up exactly where this one left off. |
